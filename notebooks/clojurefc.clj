...
;; # Learn Clojure and Football Analytics
...
;; will explore some facts about Messi and La Liga using data taken from StatsBomb/open-data
;; github [repository](https://github.com/statsbomb/open-data), an Excel spreadsheet, and a database query.

;; ## Competitions

;; First we will read a `json` file containing list of football competitions

;; We're going to post process this table a bit with ordinary Clojure
;; sequence functions to select only columns of interest
;; For now we are going to look at the following
;; - Competition Name
;; - Competition ID
;; - Country Name
;; - Season Name
...
(def competitions
  (->> (json/read-str (slurp (str "open-data/data/competitions.json"))
                      :key-fn keyword)
       (map #(select-keys % [:competition_name :competition_id :season_name :season_id :country_name]))))
